...
type InitialPuzzleMsg struct {
	...
	// NextGuideAddr is the ID of the next tour guide to follow.
	NextGuideID int `msgpack:"i"`
	// NextGuideAddr is the IP address of the next tour guide to follow.
	...
	// IntegrityHash is a HMAC hash to let next guide perform validation.
	IntegrityHash [32]byte `msgpack:"m0"`
}

// AnotherGuidePuzzle is another guide's puzzle info given to the client on the
// current stop.
...
// PuzzleSolvingMsg is another puzzle which client sends to the next guide on
// the stop.
type PuzzleSolvingMsg struct {
	...
	// How many guided tours (count of round trips) will crypto have.
	TourLength uint16 `msgpack:"L"`
	// StopNumber is the passed stop (guide) number by a client.
	StopNumber byte `msgpack:"s"`
	// PreviousGuideTS is the previous guide puzzle generating timestamp.
	PreviousGuideTS time.Duration `msgpack:"t_{s-1}"`
	// PreviousIntegrityHash is the previous integrity HMAC hash from the previous guide.
